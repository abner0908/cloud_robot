...
def callback(msg_sub):
    bridge = CvBridge()
    try:
        cv_img = bridge.imgmsg_to_cv2(msg_sub, "bgr8")
    ...
    pub = rospy.Publisher("/cloud/detect/people", Image, queue_size=1)

    try:
        msg_pub = bridge.cv2_to_imgmsg(cv_img, "bgr8")
        msg_pub = add_header(msg_pub, msg_sub)
    except CvBridgeError as e:
        print(e)
...
def add_header(msg_pub, msg_sub):
