...
class FaceDetection:
    ...
    def __init__(self, topic, show_video, compute_index, total_nodes, before):
    ...
    def handle_sub(self):
        ...
        self.pub = rospy.Publisher(self.topic_pub, FaceDetect, queue_size=1)
...
stars = '*' * 5
help_msg = stars
help_msg += "cv_detect_face.py [-w (show video)][-b (timing before detecting)]"
help_msg += "[-t <topic_name>][-i <compute index>][-o <total node number>]"

if __name__ == '__main__':
    try:
        opts, args = getopt.getopt(sys.argv[1:], 'bhwt:i:o:', '')
    ...
    fd = FaceDetection(topic, should_show_video,
                       compute_index, total_nodes, before)
    fd.run()
